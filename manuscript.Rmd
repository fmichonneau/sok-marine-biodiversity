...
These patterns of spatial distribution for the species richness are strongly confounded by the sampling effort. Investigating the relation between the number of samples and the number of species for each cell of the map (Fig. \@ref(fig:correlation-sampling)) reveals that it takes about 100 records per cell before there is clear redundancy in the sampled species (each record represent a different species).
...
The inadequacy of the sampling effort compared to the diversity of the fauna is also reflected by the fact that `r prop_singleton_species`% of the species are known from a single record. Another indication of this inadequacy is the relation between the number of collected specimen and the number of species recorded through time (Fig. \@ref(fig:idigbio-through-time)). Initial explorations in the late ninetieth century lead to an initial exponential growth in the knowledge of the fauna, that tapered off, until the 1970's when intensive collecting efforts led to an increase in the recording of new taxa (Fig. \@ref(fig:idigbio-through-time)). Since then, the slowing down of collections after the 1970s has been accompanied with a slowing down in the discovery of new records: `r n_spp_comparison$n_spp_2017_2000` species have been documented for the first time since 2000 (`r format_output(n_spp_comparison$n_spp_2017_2000/18)` species.year^-1^), while `r n_spp_comparison$n_spp_1970_1960` have been documented between 1960 and 1970 (`r format_output(n_spp_comparison$n_spp_1970_1960/11)` species.year^-1^). The strong correlations between (1) the number of samples collected and the species richness (Fig. \@ref(fig:correlation-sampling)).; (2) the number of specimens collected per year and the number of species recorded for the first time (Fig. \@ref(fig:idigbio-through-time)C), suggest that any new sampling effort will lead to new observations and new species. Coordinated large-scale biodiversity expeditions continue to be needed to improve our understanding of the diversity of species and their spatial distributions. While less explored than the US, a recent study in Western Australia reported that 135 samples led to 805 species of crustaceans and polychaetes, 94.6% and 72% of which were undescribed [@Poore2014], highlighting the potential for discovery in the oceans.

```{r idigbio-through-time, fig.cap='Cumulative number of samples (A) and species (B) of marine invertebrates recorded through time based on iDigBio records. Relation between number of samples collected, and number of species recorded for the first time (C), each point represents numbers for a given year. (\\#fig:idigbio-through-time)'}
knitr::include_graphics("figures/idigbio_through_time.pdf")
```

Predicting the geographic range of species, and how they will be impacted by global change has undergone important advances recently [@xxxx]. However, it is generally recognized that to obtain reliable results such efforts require to have accurate geographic information for a minimum of 10 specimens but having at least 30 is preferred [@xxxx]. Based on iDigBio data, `r prop_10_specimens_species`% of species have less than 10 specimens and `r prop_30_specimens_species`% have less than 30 specimens. This suggests that our understanding of the geographic range of species is limited as most species are known from only a few locations, while efforts to estimate their current and future ranges based on occurrence data are rendered moot by the low number of data points available.
...
To assess the status of taxonomic expertise in the US, we calculated the proportion of specimens identified at the species level through time. For all taxa, excepts for molluscs, there is a consistent decline, indicating that specimens collected recently remain for the most part unidentified in collections (Fig.  \@ref(fig:identification-through-time)). For diverse groups such as annelids or arthropods, the proportion of specimens identified at the species-level is now below 30%. The number of scientists describing new species might be increasing globally [@Costello2013], but taxonomic experts continue to be needed to identify specimens recently collected.
...
The proportion of species that have DNA barcodes available is relatively constant across the better studied phyla at the scale of the EEZ. It varies between 12% for bryozoans to 28% for cnidarians (\@ref(fig:proportion-barcoded)). The proportions for species known to occur in the Gulf of Mexico are usually similar than for the US, while typically a higher fraction of species from the Pacific Northwest are available. In 2012, it was estimated that, at a global scale, less than 3% of the marine species had been sequenced [@Appeltans2012], and that this number was even lower in some areas (e.g. 2.6% for the Antarctic [@Grant2009]). This suggests that even as many additional species have had their DNA barcodes made available since these studies, the US fauna is generally better characterized than other parts of the world.
...
```{r proportion-barcoded, fig.cap='Proportion of species known to occur in the US EEZ based on iDigBio records, in the Gulf of Mexico [@Tunnell2009], in the Pacific Northwest[@Kozloff1987], that have publicly available DNA barcodes. Numbers next to the bars indicate the number of species for which DNA barcodes are available. (\\#fig:proportion-barcoded)'}
...
For both the spelling of scientific names, and the geographic information, computing tools can be developed to detect issues and propose solutions. There are opportunities to develop computing services that use publicly available data to flag and fix these issues directly in collection databases. It however requires that resources are made available to museum collections to address these issues.
...
Crowd-sourced data and citizen science initiatives have been proposed to augment data about species abundance and distributions [@Pimm2014]. For marine taxa, beyond the application to near-shore habitats on easily recognizable species [@Delaney2008], large-scale efforts are currently not possible given the specialized equipment needed to sample most marine habitats and the expertise required to identify species. There are however opportunities for citizen science to improve biodiversity data, educate and engage the general public with marine conservation issues. For instance it has been successfully used to classify plankton images [@Robinson2017], transcribe hand-written museum labels [@Hill2012], or be involved in bioblitzes [@Ballard2017]. These initiatives however involve a lot less people compared to projects related to birds.
