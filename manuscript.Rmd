...
Similarly, a significant fraction of museum records have missing or inaccurate geographic information associated with them. Geographic information might be available in other forms, but need to be digitized. While tools have been developed to convert names of locales into GPS coordinates on land [@rios2010geolocate], similar approaches need to be develop for the sea.

For both the spelling of scientific names, and the geographic information, computing tools can be developed to detect issues and propose solutions. There are opportunities to develop computing services that use publicly available data to flag and fix these issues directly from collection databases records. It however requires that resources are made available to museum collections in order to address these issues directly in collection databases.
...
Another venue to improve the quality of the data in biodiversity information systems, is to develop better incitives from journal editors and funding agencies for scientists to publish their occurrence records in databases alongside their papers as it is done for genetic data with GenBank [@Huang2011e].
