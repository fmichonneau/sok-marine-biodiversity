...
The proportion of species that have DNA barcodes available is relatively constant across the better studied phyla at the scale of the EEZ. It varies between 12% for bryozoans to 28% for cnidarians. The proportions for species known to occur in the Gulf of Mexico are usually similar than for the US, while typically a higher fraction of species from the Pacific Northwest are available. Geographic information is missing for most records (`r format_output(100*stat_bold$p_no_coords)`% of the `r format(stat_bold$n_total_records)` BOLD records analyzed here have no geographic coordinates information associated with them), and many marine invertebrate species are known to be cryptic complexes that are not as widely distributed as previously thought. Therefore specimens identified under the same species but sampled in distant locations, could represent cryptic species. It is thus crucial to ensure that accurate geographic information is provided when DNA barcodes are published, and that additional samples are collected, to capture more accurately their geographic range and the extent of their genetic diversity. This cryptic diversity is reflected in the fact that `r stat_bold$n_spp_multi_bin` species out of `r stat_bold$n_spp` include more than 1 BIN (BOLD Index Number, operational taxonomic units that closely resemble species [@Ratnasingham2013]).
