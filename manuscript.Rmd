...
Regional faunistic lists assembled and curated by taxonomic experts provide a comprehensive and detailed account of species that are known to occur in a given geographic area. We assessed how faunistic lists assembled over decades by specialists compare with records in public databases for the same geographic area. We parsed faunistic lists to extract marine invertebrates from the Gulf of Mexico [@Tunnell2009] and from the Pacific Northwest [@Kozloff1974], leading to `r format(sum(gom_worms$is_marine, na.rm=TRUE), big.mark=",")` and `r format(sum(kozloff_worms$is_marine, na.rm=TRUE), big.mark=",")` species respectively. Our goals were (1) to compare  the species lists for these geographic areas assembled by experts with the records found in databases; (2) to assess the proportion of species included in the expert lists that have records in public databases.

To compare the faunistic lists from the Gulf of Mexico and the Pacific Northwest with records in public databases, we queried the iDigBio (https://portal.idigbio.org) and OBIS (http://www.iobis.org/) databases for all records they included in the same geographic boundaries as the lists. We filtered the results to only include the same higher taxonomic level taxa as in the faunistic lists, and kept only records for which the species could be matched to a species name in WoRMS. For the Gulf of Mexico, we retrieved  `r format(dplyr::n_distinct(idigbio_gom_records$worms_valid_name), big.mark=",")` species in iDigBio (from $n =$  `r format(nrow(idigbio_gom_records), big.mark=",")` records), and `r format(dplyr::n_distinct(obis_gom_records$worms_valid_name), big.mark=",")` species in OBIS ($n=$ `r format(nrow(obis_gom_records), big.mark=",")` records). For the Pacific Northwest, we retrieved `r format(dplyr::n_distinct(idigbio_pnw_records$worms_valid_name), big.mark=",")` species in iDigBio ($n=$ and `r format(nrow(idigbio_pnw_records), big.mark=",")`) and `r format(dplyr::n_distinct(obis_kozloff_records$worms_valid_name), big.mark=",")` species in OBIS ($n=$ `r format(nrow(obis_kozloff_records), big.mark=",")`). Overall, the number of species known from the Gulf of Mexico match the number estimated by experts (`r format(11150-1541-434, big.mark=",")` species [@Fautin2010]). For the Pacific coast, this number is lower than the estimate (`r format(8374-909-134, big.mark=",")` species [@Fautin2010]), but this estimate included species from California and from deeper areas that were not included in Kozloff's faunistic list.

For the Gulf of Mexico, the majority of the species included in the expert faunistic list are recorded in both iDigBio and OBIS (`r format_output(species_in_common$n_spp_gom_idigbio_obis/species_in_common$n_spp_gom*100)` %), or only OBIS (`r format_output(species_in_common$n_spp_gom_obis/species_in_common$n_spp_gom*100)` %) (Fig. \@ref(fig:database-coverage)). For the Pacific Northwest, these proportions were much lower with only `r format_output(species_in_common$n_spp_kozloff_idigbio_obis/species_in_common$n_spp_kozloff*100)`% of the species from the regional list with records in OBIS and iDigBio, and `r format_output(species_in_common$n_spp_kozloff_obis/species_in_common$n_spp_kozloff*100)`%  in OBIS only (Fig. \@ref(fig:database-coverage)). Actually, the majority of the species in the faunistic list are not recorded in any of the public databases. Overall, there is more overlap among the data sources for the Gulf of Mexico than the Pacific Northwest.

```{r compare_database_coverage, fig.cap='Overlap between the regional species list (list), records in iDigBio (idigbio), records in OBIS (obis), and records in BOLD (bold) for the Gulf of Mexico (left) and the Pacific Northwest (right). This plot is generated using the UpSet technique[@Lex2014], which uses a matrix-based layout to represent intersections of sets and their sizes. The rows represent the sets, the columns represent the intersections, and each set that is part of the intersection is marked with a black-filled circle in the cells underneath. Black lines connect the sets that are part of the intersections. (\\#fig:database-coverage)', echo=FALSE}
knitr::include_graphics("figures/database_coverage.pdf")
```

We investigated the number of species for each phylum that contributed to the discrepancies between the regional species lists and the database records. We found that the better studied phyla contributed the most to the differences across the data sources. OBIS tends to be closer to the full record set, but iDigBio has more species for the Gulf of Mexico that are not included in the list (Fig. \@ref(fig:database-overlap)). Many species of mollusks, arthropods, and annelids, known to occur in the Gulf of Mexico or in the Pacific Northwest, have not digital records for them in these geographic areas.
...
Species that are included in the list but do not have records in databases could be (1) rare: they have never been collected or have been collected only a few times, and the corresponding records have not been digitized yet; (2) difficult to preserve: large, very small, or fragile species are deposited in museums less frequently; (3) are in the database under a different name: the species has been synonimized or re-identified since the publication of the list under a different name that may be included in the database; (4) at the limit of their range: the species were included in the list because they are known to occur within the boundaries of the geographic area, but they have never been collected within these geographic boundaries; (5) do not have geographic coordinates associated with them: some of these species can be in the database but without georeferenced datapoints and therefore were not returned in our queries.

Untangling the relative contribution of these factors to explain the discrepencies is difficult as it would require to know the criteria used by the experts who have assembled the lists. However, by querying iDigBio we found that for the `r format(not_really_in_database$idig_gom_n_spp_not_in_db, big.mark=",")` species that do not appear within the geographic range of the Gulf of Mexico but are on the list, `r format_output(not_really_in_database$idig_gom_p_spp_not_in_db*100)`% are truly absent from iDigBio, while for the `r format(not_really_in_database$idig_gom_n_spp_no_coords, big.mark=",")` species that appear in iDigBio, `r format_output(not_really_in_database$idig_gom_p_spp_no_coords*100)`%  do not have geographic coordinates associated with them, and for the ones that do have geographic coordinates, `r format_output(not_really_in_database$idig_gom_p_spp_within_usa*100)`% ($n=$  `r format(not_really_in_database$idig_gom_n_spp_within_usa, big.mark=",")`) fall within the US waters. These numbers indicate that the lack of geographic coordinates associated with the specimens in the database contribute at least significantly to the discrepency, but that a large fraction of these species have no digital records of their occurrence in the Gulf of Mexico.

Species that are in the database but not in the faunistic list could be: (1) records not considered by the authors of the checklist: either because they were not accessible or known to the authors; (2) records erroneously excluded by the author of the checklist; (3) records that have been discovered since the list has been assembled; (4) records with incorrect identification: records identified as a species that does not occur in the area; (5) records with incorrect geographic coordinates: their associated geographic information in the database places them in the Gulf of Mexico or the Pacific Northwest, but they were actually collected elsewhere.

We investigated the proportion of species discovered since the publication of the faunistic lists using iDigBio records. They represent `r sum(recently_collected$gom_in_idig$min_year > 2001)` species for the Gulf of Mexico, and `r sum(recently_collected$pnw_in_idig$min_year > 1987)` species for the Pacific Northwest. While errors in geographic coordinates cannot be completely ruled out, it is more likely that the main contributing factor was that the authors of these checklists were not aware of the presence of these species based only on museum records, as these lists were mostly assembled from publications, and only a small fraction of museum records are used in publications.

### Comparison with Regional Lists by Species Names

To assess whether species included in regional faunistic lists but absent from the geographic area in public databases nevertheless occur elsewhere, we queried the iDigBio and OBIS using the species names included in these faunistic lists.

We extracted the `r format(sum(gom_worms$is_marine, na.rm = TRUE), big.mark=",")` marine invertebrates species from the faunistic list of the Gulf of Mexico, and `r format(sum(kozloff_worms$is_marine, na.rm=TRUE), big.mark=",")` species from the Pacific Northwest. We then looked for the number of species from these faunistic lists that are known to occur within the geographic area, or somewhere else in the world. Overall,
OBIS had georeferenced records for most of the species that were included in the faunistic lists for the Gulf of Mexico. However, even for the better studied phyla about 75% of the species have records from the same geographic area, while it is less than 50% for the Pacific Northwest.
...
Furthermore, there are important discrepancies in our level of knowledge across phyla. While larger, and accessible groups represent the bulk of the records for marine invertebrates in museum collections, the small organisms, including representatives of the meiofauna are virtually absent. Of the `r n_idigbio_phyla` phyla occurring in iDigBio, `r records_rare_phyla$n_rare_phyla` are represented by less than 500 records, and make up `r records_rare_phyla$prop_rare_phyla`% of the total number of records. Yet, these undersampled taxa are likely to play an important role in the detection of changes in environmental quality due to their size, and physiological vulnerability [@Bik2012]. Additionally, the impact of these changes on higher trophic levels and ecosystems is largely unexplored.

To assess the status of taxonomic expertise in the US, we calculated the proportion of specimens identified at the species level through time. For all taxa, excepts for molluscs, there is a consistent decline, indicating that specimens remain for the most part unidentified in collections \@ref{fig:identification-through-time}. For diverse groups such as annelids or arthropods, the proportion of specimens identified at the species-level is now below 30%. The number of scientists describing new species might be increasing globally [@Costello2013], but local experts are needed to keep identifying the species that get collected.
...
Crowd-sourced data and citizen science initiatives have been proposed to augment data about species abundance and distributions [@Pimm2014]. For marine taxa, beyond the application to near-shore habitats on easily recognizable species [@Delanay2008], large-scale efforts are currently not possible given the specialized equipment needed to sample most marine habitats and the expertise required to identify species. There however opportunities to utilize citizen science to improve biodiversity data and engage the general public into marine conservation issues. For instance it has been successfully used to classify plankton images [@Robinson2017], transcribe hand-written museum labels [@Hill2012], or be involved in bioblitzes [@Ballard2017]. These initiatives however involve a lot less people compared to projects related to birds.
...
The list of species for the Gulf of Mexico was obtained by downloading CSV files for each animal phylum from http://e-gulf.org/ [@egulf]. This database includes the species listed in a comprehensive biotic inventory of the Gulf of Mexico [@Felder2009].

The list of species for the Pacific Northwest was downloaded from:  https://www.eopugetsound.org/species/custom-lists/307 (accessed on July 5th, 2017) as a Microsoft Excel spreadsheet. This is a digital version of the species included in [@Kozloff1974].

To search by geographic area, we obtained the boundaries of the Exclusive Economic Zone for the United States from marineregions.org (mrgid:8456) [@mregions]. For the Pacific Northwest, Kozloff did not specify the exact geographic extent covered by his book. We infered a polygon that encompass part of the Oregon coast to the northern parts of Vancouver island. For the Gulf of Mexico, we used the same geographic area as the one defined in [@Tunnell2009] by modifying manually the eastern boundaries of the shape file for the Gulf of Mexico provided by marineregions.org (mrgid:4288). These geographic areas were split into squares of 0.2°x0.2° which were used to query the web API of iDigBio and OBIS using the R packages ridigbio and robis respectively. The resulting occurences were then clipped to only include records that fell within the EEZ boundaries.

The species names used in queries or obtained as results were checked against the World Registry of Marine Species (WoRMS). Whenever possible, species names were matched and the current valid names for the species were used. This ensured that the same species listed under different synonyms was only counted once. Species that could not be matched in WoRMS, and species listed as non-marine in WoRMS were not included in subsequent analyses. The R package taxizesoap was used to query the WoRMS database.

To assess the proportion of species for which DNA barcodes are available, we used the current valid names for all records to query the Barcode of Life Datasystem (BOLD) using the R package bold. When no records where found for a species, we looked up for alternative matches using its synonyms as listed in WoRMS.

Figure \@ref(fig:database-coverage) was generated using the R package UpSetR [@Conway2017] based on the UpSet technique [@Lex2014].
