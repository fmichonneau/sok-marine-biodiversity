...
We combined `r nrow(idigbio_records)` museum records for marine invertebrates collected in the EEZ waters of the continental US representing `r length(unique(tolower(idigbio_records[["data.dwc:phylum"]])))` phyla and `r length(unique(idigbio_records$worms_valid_name))` species. Most samples were collected on the continental shelf (Fig. \@ref(fig:map-diversity-sampling)A). For the East coast, species richness is the highest in South Florida but it is relatively uniform along the coastline. For the West coast, the Salish Sea, the Bay Area, and Southern California have the highest richness (Fig. \@ref(fig:map-diversity-sampling)B).
