...
The proportion of species that have DNA barcodes available is relatively constant across the better studied phyla at the scale of the EEZ. It varies between 12% for bryozoans to 28% for cnidarians. The proportions for species known to occur in the Gulf of Mexico are usually similar than for the US, while typically a higher fraction of species from the Pacific Northwest are available. In 2010, it was estimated that a global scale, less than 10% of the species for each marine phylum had been sequenced [@Radulovici2010], and that this number was even lower in some areas (e.g. 2.6% for the Antarctic [@Grant2009]). This suggests that even as many additional species have had their DNA barcodes made available since these studies, the US fauna is generally better characterized than other parts of the world.
...
For both the spelling of scientific names, and the geographic information, computing tools can be developed to detect issues and propose solutions. There are opportunities for services using publicly available data to be developed to make curators and collection managers aware of these issues so that they can fixed at the source. It however requires that resources are available to museum collections to address these issues.

Increasing the computing literacy of the staff of museum in order for them to manipulate and use the data from their collection database is equally critical. As data across institutions get pooled into the large biodiversity information systems, it is crucial that data standards such as Darwin Core are embraced by the community.

Another potential venue to improve the quality of the data in biodiversity information systems, is better incitives for scientisits to publish their occurrence records alongside their papers as it is done for genetic data with GenBank [@Huang2011].
...
Samples collected during these expeditions need to be photographed to document live appearance that would otherwise be lost during preservation. We also need to capture metadata associated with the specimens to be able to understand better the physical characteristics of the habitats for these species. Specimens need to be sub-sampled to create material suitable for genomic analysis. Parts of data and specimen collection can be automated and captured digitally directly in the field speeding up data collection.

Coupling these more traditional approaches of biodiversity documentation with metabarcoding is a promising approach to screen an habitat for its potential for species that do not yet occur in public databases. <!-- explain why better --> Metabarcoding can complement and take advantage of the DNA barcodes already available. Additionally, in the future, data generated by metabarcoding efforts can be re-interpreted as more DNA barcodes associated with correctly identified specimens become available. Building the library of DNA barcodes in parallel of metabarcoding efforts ensures that matches in the database will correspond to actual specimens deposited in natural history collections; ruling out possible artifacts of the method. Information about species identity and distribution will thus build on existing natural history data rather than being limited to matching DNA sequences across studies.

Assembling species lists is a necessary but not sufficient step to understand biodiversity. Beyond counting the number of species, we need to understand better the geographical distribution of species, their abundance, and their biology. In order to predict the impact of climate change and other anthropogenic perturbations on species distributions, we need to collect more occurences for species. One potential challenge for this is to be able to convey taxonomic concepts for mostly unamed species. Developing robust identifiers that can be used across biodiversity information systems, and potentially anchored in results from DNA barcoding or other molecular techniques, is needed to collaboratively build our knowledge of species distributions.

Beyond their utility in documenting diversity, metabarcoding is also a promising technology to monitor changes in communities and for understanding the functioning of ecosystems. For instance, despite our limited knowledge of the diversity of communities in the meiofauna, metabarcoding could be used to detect and documents changes in species assemblages following the Deepwater Horizon Oil Spill [@Bik2012]. Metabarcoding has also been used to reveal important differences in the diet of 3 related species of fishes that co-occur [@Leray2015b], providing a much more detailed understanding of trophic relations.

However, taxonomists and experts are needed to collect, sort, process, and identify these samples and the data that come along with them. Even if technological advances can greatly speed up the documentation of species and their distributions, trained personnel is needed. Taxonomic expertise is dwindling, and there are now only a handful of US natural history museums with active marine invertebrates collections.
...
Digitization efforts are catching up on an already fragmented and vastly incomplete records.
