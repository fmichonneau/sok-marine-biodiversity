...
get_idigbio_stats <- function(map_eez) {
  bbox_eez <- lawn::lawn_bbox(map_eez)
  idig_non_fossil <- ridigbio::idig_count_records(list("basisofrecord" = "preservedspecimen"))
  idig_non_fossil_eez <- ridigbio::idig_count_records(
    list(
      "basisofrecord" = "preservedspecimen",
      "geopoint" = list(
  ...
  list(
    ...
    eez_no_fossil = idig_non_fossil_eez
  )
}
...
plot_spp_not_in_idigbio <- function(koz_not_idig) {
  koz_not_idig %>%
    ...
    geom_text(aes(
      x = reorder(phylum, not_in_idigbio), y = .9,
...
plot_spp_not_in_idigbio_combined <- function(koz_not_idig, gom_not_idig) {
  ...
  res %>%
    ...
    mutate(
      n_p = if_else(grepl("^p", idig_status), "p",
