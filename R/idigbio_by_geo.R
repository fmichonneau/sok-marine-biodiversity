...
coords_to_query <- function(coords) {
  qry <- lapply(seq_len(nrow(coords)), function(i) {
    list(
      basisofrecord = "PreservedSpecimen", # only extant specimens (no fossils)
...
add_worms_to_db <- function(db_table) {
  ...
  if (grepl("idigbio", db_table)) {
    ...
    .Names = c(
      "scientificname", "cleaned_scientificname", "worms_id",
      "worms_valid_name", "valid_worms_id", "is_fuzzy", "rank", "is_marine",
    ...
    .Names = c(
      "aphiaid", "worms_id", "is_marine",
      "worms_valid_name", "valid_worms_id",
      "rank", "is_fuzzy", "worms_phylum",
      "worms_class", "worms_order",
      "worms_family", "phylum", "worms_kingdom"
    )
    )
  } else {
    stop(glue::glue("invalid table name: {db_table}."))
  }

  wrm_res %>%
    dplyr::mutate(worms_kingdom = add_kingdom(worms_id)) %>%
    dplyr::copy_to(db, .,
      name = glue::glue("{db_table}_species"), temporary = FALSE,
  ...
  purrr::map(
    list(
      "worms_kingdom", "worms_phylum", "worms_class",
      "worms_order", "worms_family", "worms_valid_name", "worms_id", "valid_worms_id", "rank"
    ),
  ...
  dbExecute(db, glue::glue(
    "UPDATE {db_table}_worms ",
    "SET (worms_valid_name, worms_id, valid_worms_id, is_marine, rank, worms_kingdom, worms_phylum,",
    "     worms_class, worms_order, worms_family) = ",
    "(SELECT worms_valid_name, worms_id, valid_worms_id, is_marine, rank, worms_kingdom, ",
...
extract_inverts_from_db <- function(db_table, geo) {
  ...
  db_worms %>%
    ...
    add_sub_kingdom() %>%
    dplyr::filter(
      sub_kingdom == "animalia - invertebrates",
      is_marine
    ) %>%
    ## select needed columns
    dplyr::select(
      uuid, phylum, class, order, family, genus, scientificname,
      decimallatitude, decimallongitude, datecollected, institutioncode,
      within_eez, within_gom, within_pnw,
      worms_phylum, worms_class, worms_order, worms_family,
      worms_valid_name, worms_id, valid_worms_id, is_marine, rank,
...
check_phyla_in_db <- function(db, list_phyla) {
  ...
  if (!all(phyla_in_db %in% list_phyla[["phylum"]])) {
    stop(
      "Some phyla in the database are not in the dictionary: ",
...
mms_samples_through_time <- function(idig_mms) {
  idigbio_mms_records %>%
    ...
    dplyr::ungroup() %>%
    ggplot() +
    geom_col(aes(x = year, y = n, fill = worms_phylum))
}
