...
plot_samples_vs_spp_through_time <- function(knowledge_through_time) {
  ...
  res <- knowledge_through_time %>%
    ...
    dplyr::group_by(year) %>%
    dplyr::summarize(
      n_spp = sum(n_new_spp, na.rm = TRUE),
      n_samples = sum(n_samples, na.rm = TRUE)
    ) %>%
    dplyr::mutate(period = cut(year,
      breaks = seq(1850, 2030, by = 20),
      labels = paste(seq(1850, 2010, by = 20),
        c(
          seq(1870, max(year, na.rm = TRUE), by = 20),
          max(year, na.rm = TRUE)
        ),
        sep = "-"
      ), right = FALSE
  ...
  res %>%
    ...
    geom_point() +
    geom_line(
      data = tibble(samples = 1:100000, species = samples),
      aes(x = samples, y = species), color = "#2b2b2b", linetype = 2,
      inherit.aes = FALSE
    ) +
    geom_line(
      data = tibble(samples = 1:100000, species = samples / 100),
      aes(x = samples, y = species), color = "#2b2b2b", linetype = 3,
      inherit.aes = FALSE
    ) +
    geom_text(
      data = tibble(
        x = c(1.39, 139), y = c(1.5, 1.5),
        label = c("1:1", "1:100")
      ),
...
plot_samples_vs_spp_std <- function(knowledge_through_time) {
  ...
  knowledge_through_time %>%
    ...
    dplyr::filter(phylum %in% phy_to_keep) %>%
    dplyr::mutate(
      prop_new_spp = n_new_spp / n_samples,
...
plot_new_effort <- function(knowledge_through_time) {
  ...
  knowledge_through_time %>%
    ...
    ggplot(aes(
      x = n_samples, y = n_new_spp, group = phylum, colour = year,
...
plot_institutions_through_time <- function(idig_records) {
  ...
  idig %>%
    ...
    geom_col() +
    facet_wrap(~institutioncode, scales = "free_y") +
...
plot_identification_level_through_time <- function(idig_records) {
  idig_records %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(
      worms_phylum = capitalize(worms_phylum),
