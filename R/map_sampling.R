...
add_rastercell <- function(d, raster) {
  d %>%
    dplyr::mutate(rastercell = mapply(
      function(x, y) {
...
data_map <- function(recs, raster) {
  ...
  h_index <- recs %>%
    ...
    dplyr::group_by(rastercell) %>%
    dplyr::mutate(
      n_records_per_cell = sum(n_records_per_spp),
...
make_heatmap <- function(gg_r, title, base_map) {
  ...
  state <- map2SpatialPolygons(state,
    IDs = IDs,
  ...
  ggplot() +
    ...
    scale_fill_gradient2(
      low = "#5E98AE", mid = "#E3C94A", high = "#D5331E",
      midpoint = mid_point,
...
make_heatmap_by_phylum <- function(recs, file, raster, base_map) {
  if (exists("worms_phylum", recs)) {
    ...
    stop("can't find phylum nor worms_phylum")
  }
...
get_bubble_map_data <- function(recrds, raster, out = "data/bubble_map_data.csv") {
  ...
  dplyr::bind_rows(ts_l, .id = "year_frame") %>%
    ...
    dplyr::mutate(
      year_frame = as.integer(year_frame),
...
bubble_map <- function(tf, file = "/tmp/sampling_map.mp4") {
  ...
  us_bathy <- suppressMessages(getNOAA.bathy(
    lon1 = -128, lon2 = -60,
    lat1 = 22, lat2 = 51,
    keep = TRUE
  )) %>%
  ...
  p <- ggplot(data = tf, aes(frame = full_frame)) +
    ...
    geom_point(
      data = dplyr::filter(tf, !is.na(value)),
