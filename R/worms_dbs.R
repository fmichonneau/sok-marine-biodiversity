insert_worms_ids <- function(wrm_tbl) {
    ...
    dbplyr::db_copy_to(db, "tmp_names_to_match", .wrm_tbl, temporary = TRUE,
      overwrite = TRUE,
    ...
    res <- dbExecute(db, glue::glue("CREATE TEMPORARY TABLE tmp_worms_ids ",
    ...
    dbExecute(db, glue::glue("INSERT INTO worms_ids ",
                             "SELECT * FROM tmp_matched_names;"))

    dbExecute(db, "ANALYZE worms_ids;")
    dbSendQuery(db, glue::glue("SELECT * FROM tmp_names_to_match ",
