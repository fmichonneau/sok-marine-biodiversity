...
plot_database_overlap <- function(comp_db) {
  ...
  gg <- ggplot(data_for_plot, aes(x = phylum, color = database)) +
    ...
    geom_point(aes(y = -offset + not_in_geo),
      size = .7,
      position = position_dodge(width = dodge_width)
    ) +
    ...
    geom_point(aes(y = offset + not_in_list),
      size = .7,
      position = position_dodge(width = dodge_width)
    ) +
...
get_not_really_in_database <- function(database_overlap) {
  ...
  idig_gom_n_spp_not_in_db <- length(setdiff(
    tolower(idig_gom_not_in_db$worms_valid_name),
...
calc_prop_missing_depth <- function(obis) {
  list(
    p_missing = mean(is.na(obis$depth) &
