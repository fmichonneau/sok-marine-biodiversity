...
get_kingdom_worms_stats <- function(worms_stats) {
  ...
  wrm <- worms_stats %>%
    ...
    dplyr::mutate(sub_kingdom = case_when(
      tolower(kingdom) %in% c(
        "fungi", "protozoa", "bacteria",
        "archaea", "viruses", "biota incertae sedis"
      ) ~ "others",
...
if (FALSE) {
  ...
  idigbio_kingdom_stats("us_idigbio_clean") %>%
    ...
    dplyr::filter(worms_phylum %in% c(
      "chordata", "mollusca", "arthropoda", "annelida",
    ...
    xlim(c(0, 100)) # position = "dodge") #+ ylim(c(0, 750)) + #scale_y_log10() +
  ...
  idigbio_kingdom_stats("us_idigbio_clean") %>%
    ...
    scale_y_log10() +
    coord_flip() #+ facet_wrap(~ worms_kingdom, scales = "free")
